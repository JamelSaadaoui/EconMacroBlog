function [BVAR] = bvar_misspooled(y,lags,options)
...
% Interpolate missing values only if handle_missing is NOT activated
mixed_freq_on = 0;
if any(any(isnan(y))) %== 1
    if isfield(options, 'handle_missing') && options.handle_missing == 1
        warning('Skipping interpolation: Handling missing data by unit as specified.');
    else
        if any(sum(isnan(y),2) == ny)
            warning('Cannot activate the Mixed Frequency BVAR. Data contains rows that have only ''nan''s. Each variable is interpolated individually.')        
        else
        ...
        % Interpolate the variables that have NaNs
        T = 1:1:length(y);
...
if nunits == 1
    ...
    yy = []; XX = [];
    for nunit = 1 : nunits
        y_unit = y(idx, :, nunit);  % Extract unit-specific data

        if isfield(options, 'handle_missing') && options.handle_missing == 1
            % Remove rows with NaNs before constructing yy and XX
            valid_rows = all(~isnan(y_unit), 2); % Keep only rows where all variables are non-missing
            y_unit = y_unit(valid_rows, :);
        end

        % Construct yy and XX for this unit
        [yy_unit,XX_unit] = YXB_(y_unit, lags, [nx timetrend]);

        % Append to pooled dataset
        XX = [XX; XX_unit];
        yy = [yy; yy_unit];
    end
end
...
if nunits == 1
    ...
    varols.y = []; varols.X = [];
    for nunit = 1 : nunits
        % Extract data for current unit
        y_unit = ydata(:,:,nunit);
        % Identify and remove rows with NaN
        valid_rows = all(~isnan(y_unit), 2); % Ensure both y and x are valid
        y_unit = y_unit(valid_rows, :);
        x_unit = xdata(valid_rows,:);
        T_unit = length(x_unit);
%         tmp_varols = rfvar3(ydata(:,:,nunit), lags, xdata, [T; T], 0, 0);
        tmp_varols = rfvar3(y_unit, lags, x_unit, [T_unit; T_unit], 0, 0);
    ...
    function [posterior,var] = posterior_(y)
        ...
        if nunits == 1
            ...
            for nunt = 1 : nunits
                ...
                T_unit = length(x_unit);
        %         tmp_varols = rfvar3(ydata(:,:,nunit), lags, xdata, [T; T], 0, 0);
                tmp_var = rfvar3(y_unit, lags, x_unit, [T_unit; T_unit], 0, 0);
