% =========================================================================
% Replication of Fig. 4 in "Do Geopolitical Risks Raise or Lower
% Inflation?"
% =========================================================================
% Date:     10/10/2025
%
...
%   toolbox of Canova and Ferroni. Acts vs threats.
%
% Steps:
...
%% ========================================================================
% SECTION 1: VAR MODEL SPECIFICATION AND ESTIMATION
% =========================================================================
...
% Shocks of interest
indx_sho = [1 2];  % Indices for GPA and GPT shocks

% Select model variables by name
% Ordering: GPA, GPT, followed by macro variables
model_vars_names = {
    'GPA Country'
    'GPT Country'
    'Inflation (ppt)'
...
fprintf('\n%d variables included in the VAR model:\n', length(variables_sel));
...
% Estimate VAR
bvar2 = bvar_misspooled(DATAX_pooled, lags, options);
...
irfs_to_plot_at = bvar2.ir_draws(:, 1:options.hor, indx_sho, :);

% Normalize IRFs: scale by initial response of each shock variable
% GPA shock (shock 1): scale by initial response of GPA
scale_factor_acts = squeeze(irfs_to_plot_at(1, 1, 1, :));
scale_factor_matrix_acts = reshape(scale_factor_acts, [1, 1, 1, bvar2.ndraws]);
irfs_to_plot_at(:, :, 1, :) = irfs_to_plot_at(:, :, 1, :) ./ scale_factor_matrix_acts;

% GPT shock (shock 2): scale by initial response of GPT
scale_factor_threats = squeeze(irfs_to_plot_at(2, 1, 2, :));
scale_factor_matrix_threats = reshape(scale_factor_threats, [1, 1, 1, bvar2.ndraws]);
irfs_to_plot_at(:, :, 2, :) = irfs_to_plot_at(:, :, 2, :) ./ scale_factor_matrix_threats;

% Compute moments across posterior draws for each shock
% GPA (Acts) shock
mean_irfs_acts = mean(irfs_to_plot_at(:, :, 1, :), 4);
std_irfs_acts = std(irfs_to_plot_at(:, :, 1, :), 0, 4);

% GPT (Threats) shock
mean_irfs_threats = mean(irfs_to_plot_at(:, :, 2, :), 4);
std_irfs_threats = std(irfs_to_plot_at(:, :, 2, :), 0, 4);

% Construct 90% confidence intervals (normal approximation)
z_score = 1.645;  % 90% CI

% GPA confidence intervals
lower_conf_acts = mean_irfs_acts - z_score * std_irfs_acts;
upper_conf_acts = mean_irfs_acts + z_score * std_irfs_acts;

% GPT confidence intervals
lower_conf_threats = mean_irfs_threats - z_score * std_irfs_threats;
upper_conf_threats = mean_irfs_threats + z_score * std_irfs_threats;

%% ========================================================================
% SECTION 3: VISUALIZATION - ACTS VS. THREATS COMPARISON
% =========================================================================

fprintf('\nGenerating acts vs. threats comparison figure...\n');
...
for v = 1:num_vars
    ...
    % Plot GPA (Acts) shock - blue line with shading
    h1 = plot(time, squeeze(mean_irfs_acts(v, :)), '-b', 'LineWidth', 1.5);
    fill([time, fliplr(time)], ...
         [squeeze(lower_conf_acts(v, :)), fliplr(squeeze(upper_conf_acts(v, :)))], ...
         [0 0 1], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
    
    % Plot GPT (Threats) shock - black line with gray shading
    h2 = plot(time, squeeze(mean_irfs_threats(v, :)), '-k', 'LineWidth', 1.5);
    fill([time, fliplr(time)], ...
         [squeeze(lower_conf_threats(v, :)), fliplr(squeeze(upper_conf_threats(v, :)))], ...
    ...
    if v == 1
        legend([h1 h2], {'GPA Shock', 'GPT Shock'}, ...
...
print(fig, '.\figurespaper\fig4.pdf', '-dpdf', '-painters', '-r300');
fprintf('Done! Figure saved to .\\figurespaper\\fig4.pdf\n');