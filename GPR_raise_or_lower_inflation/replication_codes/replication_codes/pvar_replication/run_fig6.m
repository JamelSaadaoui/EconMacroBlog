% =========================================================================
% Replication of Fig. 6 in "Do Geopolitical Risks Raise or Lower
% Inflation?"
% =========================================================================
% Date:     10/10/2025
%
...
%   toolbox of Canova and Ferroni. Narrative Identification.
%
% Steps:
%   1. Estimate pooled VAR baseline specification
%   2. Compute impulse response functions (IRFs)
%   3. Estimate pooled VAR narrative identification
...
fprintf('\n========================================\n');
fprintf('BASELINE MODEL: Standard GPR Country\n');
fprintf('========================================\n');
...
%% ========================================================================
% SECTION 3: NARRATIVE MODEL (10 VARIABLES)
% =========================================================================

fprintf('\n========================================\n');
fprintf('NARRATIVE MODEL: GPR Narrative\n');
fprintf('========================================\n');
...
lags             = 1;      % Number of lags in the VAR
% Shock of interest
indx_sho = 1;  % Index for GPR Narrative shock

% Select model variables by name
% Ordering: GPR Narrative first, then GPR Country and macro variables
model_vars_narrative = {
    'GPR Narrative'
    'GPR Country'
    'Inflation (ppt)'
    'GDP (%)'
...
% Find indices of selected variables (preserving specified order)
[~, variables_sel_narrative] = ismember(model_vars_narrative, variable_labels);

% Store variable names for plotting
options.varnames = variable_labels(variables_sel_narrative);

fprintf('\n%d variables included in narrative model:\n', length(variables_sel_narrative));
for i = 1:length(variables_sel_narrative)
    fprintf('  %d. %s\n', i, options.varnames{i});
end
...
% Extract selected variables
DATAXtemp = DATAX_demean(:, :, variables_sel_narrative);

% Option to exclude specific countries by name
countries_to_find = "";  % Add country names here if needed
idx = find(ismember(cnames, countries_to_find));
countries_to_exclude_narr = idx;
...
for c = 1:NCountries
    ...
    if min(valid_obs) < min_obs_threshold
        fprintf('  Excluding %s: Min valid obs = %d (< %d threshold)\n', ...
                cnames{c}, min(valid_obs), min_obs_threshold);
        countries_to_exclude_narr = [countries_to_exclude_narr, c];
    end
end

% Validate country indices
if any(countries_to_exclude_narr > NCountries) || any(countries_to_exclude_narr < 1)
    error('Invalid country indices detected.');
end

% Create list of countries to keep
countries_to_keep_narr = setdiff(1:NCountries, countries_to_exclude_narr);

fprintf('\nFinal sample: %d countries (excluded %d)\n', ...
        length(countries_to_keep_narr), length(countries_to_exclude_narr));

% Prepare data for pooled estimation
DATAXtemp = DATAX_demean(:, countries_to_keep_narr, variables_sel_narrative);
DATAX_pooled_narr = permute(DATAXtemp, [1, 3, 2]);  % [T x Vars x Countries]

%% ------------------------------------------------------------------------
% SUBSECTION 3.3: Pooled VAR Estimation
% -------------------------------------------------------------------------

fprintf('\nEstimating narrative pooled panel VAR...\n');

% Enable missing data handling
options.handle_missing = 1;

% Estimate VAR
bvar2 = bvar_misspooled(DATAX_pooled_narr, lags, options);
...
% Extract IRFs for shock of interest
irfs_to_plot_narr = bvar2.ir_draws(:, 1:options.hor, indx_sho, :);

% Normalize IRFs: scale by initial response of GPR Country (variable 2)
% This ensures comparable scaling with baseline model
scale_factor_narr = squeeze(irfs_to_plot_narr(2, 1, 1, :));
scale_factor_matrix_narr = reshape(scale_factor_narr, [1, 1, 1, bvar2.ndraws]);
irfs_to_plot_narr = irfs_to_plot_narr ./ scale_factor_matrix_narr;

% Compute moments across posterior draws
mean_irfs_narr = mean(irfs_to_plot_narr, 4);  % Mean IRF
std_irfs_narr = std(irfs_to_plot_narr, 0, 4);  % Standard deviation

% Construct 90% confidence intervals (normal approximation)
lower_conf_narr = mean_irfs_narr - z_score * std_irfs_narr;
upper_conf_narr = mean_irfs_narr + z_score * std_irfs_narr;

%% ========================================================================
% SECTION 5: VISUALIZATION - BASELINE VS. NARRATIVE COMPARISON
% =========================================================================

fprintf('\nGenerating baseline vs. narrative comparison figure...\n');

% Configure plot appearance
time = 0:(options.hor - 1);  % Time axis (years)
fontnum = 8;                  % Font size

% Variables to plot (matching 9 macro variables)
variables_plot_pooled = 1:9;      % Baseline: all 9 variables
variables_plot_narr = 2:10;       % Narrative: variables 2-10 (skip GPR Narrative)
...
for v = 1:num_vars
    ...
    % Plot narrative IRF - black line with gray shading
    h2 = plot(time, squeeze(mean_irfs_narr(variables_plot_narr(v), :)), ...
              '-k', 'LineWidth', 1.5);
    fill([time, fliplr(time)], ...
         [squeeze(lower_conf_narr(variables_plot_narr(v), :)), ...
          fliplr(squeeze(upper_conf_narr(variables_plot_narr(v), :)))], ...
    ...
    % Labels and title (use narrative variable names for consistency)
    xlabel('Year', 'FontSize', fontnum - 1);
    title(variable_labels{variables_sel_narrative(variables_plot_narr(v))}, ...
    ...
    if v == 1
        legend([h1 h2], {'Baseline', 'Narrative'}, ...
...
print(fig, '.\figurespaper\fig6.pdf', '-dpdf', '-painters', '-r300');
fprintf('Done! Figure saved to .\\figurespaper\\fig6.pdf\n');