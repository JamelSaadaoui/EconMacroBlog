% =========================================================================
% Replication of Fig. 3 in "Do Geopolitical Risks Raise or Lower
% Inflation?"
% =========================================================================
% Date:     10/10/2025
%
...
%   toolbox of Canova and Ferroni. Baseline results
%
% Steps:
...
% Shock of interest
indx_sho = 1;  % Index of shock variable (GPR Country)
...
variables_sel = find(ismember(variable_labels, model_vars_names));
...
countries_to_find = "";  % Add country names here if needed (e.g., "United States")
...
for c = 1:NCountries
    ...
    % Identify complete observations (no NaNs across all variables)
    complete_rows = all(~isnan(yi), 2);
    num_complete_obs = sum(complete_rows);
    
    % Exclude if insufficient complete data
...
for v = 1:num_vars
    ...
    h = subtightplot(num_rows, num_cols, v, [0.1 0.08], [0.08 0.05], [0.07 0.04]);
    hold on;
    
    % Plot mean IRF
    plot(time, squeeze(mean_irfs_pooled(v, :)), '-b', 'LineWidth', 1.5);
    
    % Add confidence interval shading
    fill([time, fliplr(time)], ...
         [squeeze(lower_conf_pooled(v, :)), fliplr(squeeze(upper_conf_pooled(v, :)))], ...
...
print(fig, '.\figurespaper\fig3.pdf', '-dpdf', '-painters', '-r300');

fprintf('Done! Figure saved to .\\figurespaper\\fig3.pdf\n');